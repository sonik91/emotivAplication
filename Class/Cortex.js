...
class Cortex {
    constructor () {
        ...
        this.socket = new WebSocket(config.cortex.socketUrl)

        // read user infor
        this.user = config.cortex.user
    }
     ...
     */
    sub(streams){//cette fonction renvoie les valeur du stream des data du casque
        this.socket.on('open',async ()=>{
            await this.checkGrantAccessAndQuerySessionInfo()
